...
Kod yazan bir ajan olarak en büyük riskin **sessiz bozma**dır: derleme geçer,
ekran açılır, ama davranış farkında olmadan değişmiştir. Buna karşı:

- **Değiştirdiğini doğrula.** Projenin bir doğrulama komutu varsa (genelde
  `package.json` scripts içinde: `test`, `typecheck`, `verify`, `lint`) işi
  "bitti" demeden önce onu çalıştır. Komutu bilmiyorsan `package.json`/README'e
  bak; hiç yoksa bunu bir eksik olarak söyle.
- **Test kırıldıysa önce KODU incele.** Testin kendisini değiştirmek en son
  çaredir. Çoğu test, geçmişte yaşanmış bir hatayı ya da bilinçli bir tasarım
  kararını koruyordur; dosyanın başındaki yorum bloğu nedenini anlatır.
- **Testi geçirmek için testi zayıflatma.** `skip`, silme, beklentiyi gevşetme
  → bunlar hata gidermez, hatayı görünmez yapar. Gerçekten geçersiz kalmış bir
  beklenti varsa değiştir ama NEDEN değiştiğini yorumla yaz.
- **Davranış değiştiriyorsan testini de değiştir/ekle.** Yeni davranış test
  edilmediyse iş yarım kalmıştır.
- **DoD'ye doğrulama dahildir.** Bölüm 6'daki "bitti tanımı" cümlesi mümkünse
  çalıştırılabilir olmalı: "X yapınca Y görünüyor **ve doğrulama komutu yeşil**".
- **Kuralları koda göm.** Bir proje kuralı ("şu anahtar istemciye gitmez", "gün
  hesabı yereldir") yalnızca dokümanda yaşıyorsa er ya da geç çiğnenir. Böyle
  kuralları mümkün olduğunda otomatik doğrulanır hâle getir.
